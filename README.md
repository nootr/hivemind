...
Not production-ready yet. Client tokens, invites and peers are persisted in local SQLite state, and client tokens now have expiry, revocation and a memory scope. Production still needs audit logs, narrower scope policy, trusted sync and deployment hardening; see [production readiness](docs/architecture-v1.md#13-production-readiness).
