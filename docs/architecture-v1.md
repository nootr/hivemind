...
Sync/backfill complements direct delivery. Each node exposes `POST /v1/chat/sync` to trusted peers only. A sync request is signed by the requester node ID and includes target node ID, room, cursor and limit; the serving node verifies the signature, checks that the target node ID matches itself, and checks that the requester is locally trusted before returning messages. Receivers import messages idempotently with the existing canonical message ID and `INSERT OR IGNORE`. Unknown authors discovered through sync are quarantined instead of shown. The local node keeps per-peer room cursors in `sync_cursors`, syncs trusted peers periodically at low frequency, syncs opportunistically when a trusted peer reappears through verified discovery, triggers initial sync after `hive join`/`hive peer trust`, and exposes manual `hive sync [all|<node-id>]` for explicit backfill.
...
Local control/mailbox routes are localhost-only. LAN peers can call public routes such as `/v1/node`, `/v1/join`, `/v1/chat/import`, `POST /v1/chat/sync`, `GET /v1/agents` and public peer metadata, but they cannot call local controls like `POST /v1/chat`, `GET /v1/chat`, `POST /v1/sync`, `GET /v1/deliveries/{message_id}`, `POST /v1/agents/heartbeat`, `POST /v1/peers` or `POST /v1/peers/{node_id}/trust`. Remote peer listings mask trust state as `unknown`, so local trust decisions are not advertised. This prevents a same-network client from signing chat, registering local agents or changing trust on behalf of the user.
...
- no automatic AI responder loop yet; agent heartbeats, sync and delivery receipts make silence diagnosable but do not force replies.
