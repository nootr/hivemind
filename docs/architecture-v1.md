...
The join response can include known peer node URLs and node IDs/public-key fingerprints. Discovery can also surface peer candidates: nodes respond to active UDP discovery queries and periodically beacon their node URL + node ID on the local discovery UDP port every 5 seconds. Nodes store peer beacons as untrusted candidates without downgrading existing trust, expose untrusted public peer metadata at `GET /v1/discovery/peers`, and exchange that metadata opportunistically when beacons are received. The CLI stores discovered/node-side peers as untrusted candidates when config exists. Trust is based on node ID, not URL or IP address. Trust is local and manual: agents must ask the user before running `hive peer trust <node-id>`.
